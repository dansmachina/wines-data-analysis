...
output:
  pdf_document: 
    ...
    keep_tex: true
    fig_caption: true
abstract: A logistic regression project to determine the type of wine depending on its physicochemical attributes.
...
The data science pipeline often[^1] needs to split the original dataset into two smaller pieces: the train and test datasets. If we only evaluate our models in the same dataset, the results will be overestimated (aka overfitting). To provide honest assesments of the performance of the predictive models, we will need to validate the models using a test dataset, a partition that has not been used to build the models in order to avoid bias.

[^1]: Some statistical learning models are robust enough to do not need this division. They can infer the behaviour of the whole population from a sample if some statistical hypothesis are fulfilled.
...
log.full=glm(type~fixed_acidity+volatile_acidity
             +citic_acid+residual_sugar+chlorides
...
```{r model_comparison, fig.align="center", fig.width=6, fig.height=2.5, echo=FALSE, fig.cap="Accuracy vs. number of variables.", fig.retina=1}
...
After our analysis, we have seen that two different aproaches are possible in order to solve our classification problem, introduced at the begining of this document. 
As showed above, we have studied two models: a simpler one with two variables and 95% of accuracy and a more complex one with five variables and a 99% of accuracy.

It is interesting to notice that the model with just two variables has an easy explanation. Let's take a look at the Figure 2.


```{r two_variables_plot, fig.align="center", fig.width=6, fig.height=4, echo=FALSE, fig.cap="Total SO2 vs. density.", fig.retina=1, fig.pos="h"}
...
This plot shows the variables density and total SO2 but colored by type. As we can see, it is clear that we have an almost perfect distinction between red and white wines. That is why we are having such a great 95% accuracy in our simpler model.

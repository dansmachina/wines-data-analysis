...
(2) \ \ \ \  \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \  logit(\hat{y}) = \beta_0 + \beta_1*fixed\_acidity + \beta_2*volatile\_acidity 
  +\beta_4*residual\_sugar \\
+ \ \beta_5*chlorides + \beta_6*free\_sulfur\_dioxide  \\+ \ \beta_7*total\_sulfur\_dioxide   
 + \ \beta_8*density \\ + \ \beta_9*pH  +  \beta_{11}*alcohol
\end{aligned}
\end{equation*}
...
We can detect in Figure \ref{figure:accuracy-model} that using just 5 variables in the logistic regression mode we achieve the same accuracy (99\%) than using all the 9 variables from the full model, getting this final model:
\begin{equation*}
\begin{aligned}	
